...
- `xhrec-notify.sh`：Telegram 通知服务；读取 XhRec 实际日志与导出目录，通知开始录制、会话结束、获取失败和视频导出。
...
- 是否启用 Telegram 通知，并输入 Bot Token 与 Chat ID；
- 是否设置测试分段时长。
...
然后访问 `http://127.0.0.1:18190`。

## Telegram 通知状态

安装器选择启用通知后，会安装独立的 `xhrec-notify.service`（服务名前缀会跟随 `--service-name`）。通知器首次启动只建立日志和文件基线，不会补发历史通知；之后只处理新增的录制日志和 `.fixed.mp4` 文件。

通知组件的目标状态是：

- 🟢 主播上线并开始录制；
- 🟡 当前无法获取视频流（可能是私秀、票房、授权失效、网络故障或平台限制）；
- ⚫ 录制会话结束（可能是下播、达到时长限制或手动停止）；
- ✅ 视频完成导出。

每条消息都包含主播名或当前监控列表。XhRec 日志无法可靠区分“主播下播、达到时长限制、手动停止”，因此通知不会把所有停止事件武断地标成下播。

Telegram Token 与 Chat ID 只能保存在服务器本地受限配置中，不能提交到仓库。`config.example.env` 只提供变量名模板。

XhRec 面板的 API Token 默认保持为空，便于通过本机或 SSH 隧道管理添加主播、停止录制和修改设置；不要把面板端口直接暴露到公网。当前 XhRec 面板实际使用 HTTP，不是 HTTPS。浏览器面板语言由 XhRec 前端的浏览器语言/localStorage 决定，当前安装器不伪造语言设置。
